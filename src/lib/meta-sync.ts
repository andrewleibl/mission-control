...
//
// Shared by the manual button (POST /api/meta-sync) and the scheduled cron
// (GET /api/cron/meta-sync) so both run identical logic.
...
export type SyncResult =
  | { ok: true; synced: number; accounts: number; errors: string[]; flags: string[] }
  | { ok: false; status: number; error: string }

export async function runMetaSync(preset = 'last_30d'): Promise<SyncResult> {
  const token = process.env.META_ACCESS_TOKEN
  if (!token) return { ok: false, status: 400, error: 'META_ACCESS_TOKEN not set' }
  ...
  if (accounts.length === 0) return { ok: false, status: 400, error: 'No active ad accounts mapped' }
